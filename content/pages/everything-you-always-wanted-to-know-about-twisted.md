...
```python
value = order_food()
x = do_action(value)do_something_else(x)
```
...
`compliment_chef` is run as soon as `eat_meal` finishes. It is passed the
return value of `eat_meal`.

If `order_food` returned its result immediately, the code would look like:
...
no I/O, no subprocesses, no signals, no concurrency.
...
1.  If a callback takes a long time to execute, the `_run_callbacks` loop will block.
2.  There is no error handling.
3.  If a callback returns a placeholder itself, the callbacks of *that* placeholder will never be run
...
reader (hint: you can cheat by looking inside
[twisted/internet/defer.py](https://github.com/twisted/twisted/blob/trunk/twisted/internet/defer.py)).
...
`try`/`except`/`else`/`finally`. Luckily, Twisted has already implemented such
a placeholder, calling it a Deferred. The `when_ready` operation described
above is called `addCallback`, and is analogous to 'do this on next success'.
...
What happens is that after 'Hello' is printed, the `sleep_then_print`
...
write some data etc. In Twisted terminology, this is a *transport*. There are
transports for TCP, UDP, UNIX sockets, processes and SSL. Each of these knows
how to read and write bytes to the wire.
...
to events that the transport sends it, events like: `dataReceived`;
`connectionMade` and `connectionLost`.
...
transports (not strictly true — other parts of Twisted do the binding. Still,
it's a helpful lie). Twisted calls these *protocol factories*.
...
class ReverseLineFactory(protocol.ServerFactory):
...
a caller is concerned, a call to `reverse` simply returns a placeholder for a
